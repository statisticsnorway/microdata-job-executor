...
def _generate_epoch_dict(temporal_coverage: dict) -> dict:
    ...
    while start <= stop:
        days_since_epoch_for[start.strftime("%Y%m%d")] = str(
            (start - EPOCH).days
        )
...
def _enrich_csv(
    input_csv_path: Path, temporal_coverage: dict, data_type: str
) -> Path:
    output_csv_path = (
        input_csv_path.parent / f"{input_csv_path.stem}_enriched.csv"
    )
...
def run(input_csv_path: Path, temporal_coverage: dict, data_type: str) -> Path:
    ...
    try:
        ...
        output_file = _enrich_csv(input_csv_path, temporal_coverage, data_type)
