...
class Datastore:
    ...
    def _generate_new_metadata_all(
        ...
        new_metadata_all_dict = self.metadata_all_draft.model_dump(by_alias=True, exclude_none=True)
        del new_metadata_all_dict["dataStructures"]
        new_metadata_all_dict["dataStructures"] = [
            dataset.model_dump(by_alias=True, exclude_none=True) for dataset in new_version_metadata
        ]
        new_metadata_all = MetadataAll(**new_metadata_all_dict)
        local_storage.write_metadata_all(
            new_metadata_all.model_dump(by_alias=True, exclude_none=True), new_version
        )
        self.metadata_all_latest = MetadataAll(
    ...
    def _version_pending_operations(
        ...
        for release_update in release_updates:
            ...
            self._log(
                job_id, f"Versioning {dataset_name} with operation {operation}"
            )
    ...
    def bump_version(
        ...
        try:
            ...
            self.metadata_all_draft.rebuild(
                self.metadata_all_latest.data_structures, self.draft_version
            )
            ...
            self._log(job_id, "An unexpected error occured", "ERROR")
            self._log(job_id, str(e), "EXC", e)
            rollback_bump(job_id, bump_manifesto.model_dump(by_alias=True, exclude_none=True))
            job_service.update_job_status(job_id, "failed")
