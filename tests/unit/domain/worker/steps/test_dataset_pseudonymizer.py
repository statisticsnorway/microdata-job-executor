...
WORKING_DIR = Path(
    "tests/unit/resources/domain/worker/steps/dataset_pseudonymizer"
)
...
with open(
    f"{WORKING_DIR}/metadata_pseudonymize_unit_id_and_value.json",
    encoding="utf-8",
) as file:
    PSEUDONYMIZE_UNIT_ID_AND_VALUE_METADATA = Metadata(**json.load(file))
with open(
    f"{WORKING_DIR}/metadata_pseudonymize_value.json",
    encoding="utf-8",
) as file:
